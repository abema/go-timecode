...
func TestNewTimecodeNDF(t *testing.T) {
	...
	t.Run("29.97fps (NDF)", func(t *testing.T) {
		tc, err := NewTimecode(1799, 30000, 1001, assumeNDF)
		assert.NoError(t, err)
		assert.Equal(t, "00:00:59:29", tc.String())
		assert.Equal(t, uint64(1799), tc.Frames())
		assert.Equal(t, 60.027, math.Round(tc.Duration().Seconds()*1000)/1000)
		assert.Equal(t, int32(30000), tc.FramerateNumerator())
		assert.Equal(t, int32(1001), tc.FramerateDenominator())

		tc, err = NewTimecode(1800, 30000, 1001, assumeNDF)
		assert.NoError(t, err)
		assert.Equal(t, "00:01:00:00", tc.String())
		assert.Equal(t, uint64(1800), tc.Frames())

		tc, err = NewTimecode(1801, 30000, 1001, assumeNDF)
		assert.NoError(t, err)
		assert.Equal(t, "00:01:00:01", tc.String())
		assert.Equal(t, uint64(1801), tc.Frames())

		tc, err = NewTimecode(1800*10, 30000, 1001, assumeNDF)
		assert.NoError(t, err)
		assert.Equal(t, "00:10:00:00", tc.String())
		assert.Equal(t, uint64(1800*10), tc.Frames())
		assert.Equal(t, 600.601, math.Round(tc.Duration().Seconds()*1000)/1000)

		tc, err = NewTimecode(1800*10+1, 30000, 1001, assumeNDF)
		assert.NoError(t, err)
		assert.Equal(t, "00:10:00:01", tc.String())
		assert.Equal(t, uint64(1800*10+1), tc.Frames())

		maxFrames := uint64(24*6*(1800*10)) - 1
		tc, err = NewTimecode(maxFrames, 30000, 1001, assumeNDF)
		assert.NoError(t, err)
		assert.Equal(t, "23:59:59:29", tc.String())
		assert.Equal(t, maxFrames, tc.Frames())
		assert.Equal(t, 86486.453, math.Round(tc.Duration().Seconds()*1000)/1000)

		tc, err = NewTimecode(maxFrames+1, 30000, 1001, assumeNDF)
		assert.Equal(t, ErrTooManyFrames, err)
		assert.Nil(t, tc)
	})
	t.Run("30fps", func(t *testing.T) {
	...
	t.Run("59.94fps (NDF)", func(t *testing.T) {
		tc, err := NewTimecode(0, 60000, 1001, assumeNDF)
		assert.NoError(t, err)
		...
		assert.Equal(t, int32(60000), tc.FramerateNumerator())
		assert.Equal(t, int32(1001), tc.FramerateDenominator())

		tc, err = NewTimecode(3599, 60000, 1001, assumeNDF)
		assert.NoError(t, err)
		assert.Equal(t, "00:00:59:59", tc.String())
		assert.Equal(t, uint64(3599), tc.Frames())

		tc, err = NewTimecode(3600, 60000, 1001, assumeNDF)
		assert.NoError(t, err)
		assert.Equal(t, "00:01:00:00", tc.String())
		assert.Equal(t, uint64(3600), tc.Frames())

		tc, err = NewTimecode(3601, 60000, 1001, assumeNDF)
		assert.NoError(t, err)
		assert.Equal(t, "00:01:00:01", tc.String())
		assert.Equal(t, uint64(3601), tc.Frames())

		tc, err = NewTimecode(3600*10, 60000, 1001, assumeNDF)
		assert.NoError(t, err)
		assert.Equal(t, "00:10:00:00", tc.String())
		assert.Equal(t, uint64(3600*10), tc.Frames())
		assert.Equal(t, 600.601, math.Round(tc.Duration().Seconds()*1000)/1000)

		tc, err = NewTimecode(3600*10+1, 60000, 1001, assumeNDF)
		assert.NoError(t, err)
		assert.Equal(t, "00:10:00:01", tc.String())
		assert.Equal(t, uint64(3600*10+1), tc.Frames())

		maxFrames := uint64(24*6*(3600*10)) - 1
		tc, err = NewTimecode(maxFrames, 60000, 1001, assumeNDF)
		assert.NoError(t, err)
		assert.Equal(t, "23:59:59:59", tc.String())
		assert.Equal(t, maxFrames, tc.Frames())
		assert.Equal(t, 86486.47, math.Round(tc.Duration().Seconds()*1000)/1000)

		tc, err = NewTimecode(maxFrames+1, 60000, 1001, assumeNDF)
		assert.Equal(t, ErrTooManyFrames, err)
		assert.Nil(t, tc)
	})
	t.Run("60fps", func(t *testing.T) {
...
func TestParseTimecode(t *testing.T) {
	t.Run("ParseTimecode/29.97DF", func(t *testing.T) {
		tc, err := ParseTimecode("00:01:00;00", 30000, 1001) // DF
		assert.NoError(t, err)
		assert.Equal(t, "00:01:00;02", tc.String())
		assert.Equal(t, uint64(1800), tc.Frames())
		assert.Equal(t, ":", tc.sep)
		assert.Equal(t, ";", tc.lastSep)
	})
...
func TestAdd(t *testing.T) {
	...
	t.Run("Add/mismatch frame rate1", func(t *testing.T) {
